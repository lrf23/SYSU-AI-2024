    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[nx][ny]:
                nstate=(nx,ny)
                cost=1
                corners=tuple(x for x in state[1] if x!=(nx,ny))
                successors.append(((nstate,corners),action,cost))
...
def cornersHeuristic(state, problem):
    ...
    for s in state[1]:
        xy1=pre_s
        xy2=s
        dis.append(abs(xy1[0] - xy2[0]) + abs(xy1[1] - xy2[1]))
    w=walls.width-3
    h=walls.height-3
    h_cost=min(dis)+min(w,h)*(len(state[1])-1)
    return h_cost # Default to trivial solution
