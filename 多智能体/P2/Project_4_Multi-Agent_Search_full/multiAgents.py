...
class ReflexAgent(Agent):
    ...
    def evaluationFunction(self, currentGameState, action):
        ...
        return score+successorGameState.getScore() # 下一个状态下的分数（屏幕左下角那个）
...
class AlphaBetaAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        best_action=None
        alpha=-float('inf')#到目前为止最大价值
        beta=float('inf')#到目前为止最小价值
        max_value=-float('inf')
        for action in nextActions:
            val=self.getValue(gameState.generateSuccessor(agentIndex=0,action=action),1,0,alpha=alpha,beta=beta)
            if val is not None and val>max_value:
                max_value=val
                best_action=action
        ...
        if (len(nextActions)==0):
            ...
            if agentIndex==0:
                ...
                if depth==self.depth:
                    ...
                    return self.max_val(gameState,agentIndex,depth,alpha,beta)
            else:
                return self.min_val(gameState,agentIndex,depth,alpha,beta)
            
    def max_val(self,gameState,agentIndex,depth,alpha,beta):
        max_val=-float('inf')
        nextActions=gameState.getLegalActions(agentIndex=agentIndex)
        for action in nextActions:
            val=self.getValue(gameState.generateSuccessor(agentIndex=agentIndex,action=action),(agentIndex+1)%gameState.getNumAgents(),depth,alpha,beta)
            if val is not None and val>max_val:
                max_val=val
            if (max_val>beta):
                return max_val
...
class ExpectimaxAgent(MultiAgentSearchAgent):
    ...
    def getValue(self,gameState,agentIndex,depth):
        ...
        if (len(nextActions)==0):
            ...
            if agentIndex==0:
                ...
                return self.average_val(gameState,agentIndex,depth)
    ...
    def average_val(self,gameState,agentIndex,depth):
        total_val=0
        nextActions=gameState.getLegalActions(agentIndex=agentIndex)
        for action in nextActions:
            val=self.getValue(gameState.generateSuccessor(agentIndex=agentIndex,action=action),(agentIndex+1)%gameState.getNumAgents(),depth)
        ...
        return total_val/len(nextActions)
...
def betterEvaluationFunction(currentGameState):
    ...
    score=0
    if (len(foods)!=0):
        min_food_dis=min([manhattanDistance(food,pos) for food in foods])
        if min_food_dis!=0:
            score+=1/min_food_dis
        else :
            score+=1
    if (len(ghostStates)!=0):
        min_ghost_dis=min([manhattanDistance(gp.configuration.pos,pos) for gp in ghostStates])
        if min_ghost_dis!=0:
            score-=1/min_ghost_dis
        else :
            return 0

    
    timescore=max(scaredTime)
    return currentGameState.getScore()+score+timescore
    util.raiseNotDefined()
