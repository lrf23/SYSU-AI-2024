...
def constructBayesNet(gameState):
    ...
    "*** YOUR CODE HERE ***"
    edges.append((X_POS_VAR, HOUSE_VARS[0]))
    edges.append((X_POS_VAR, HOUSE_VARS[1]))
    edges.append((Y_POS_VAR, HOUSE_VARS[0]))
    edges.append((Y_POS_VAR, HOUSE_VARS[1]))
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    """
    "*** YOUR CODE HERE ***"
    inferenceResult = inference.inferenceByVariableElimination(bayesNet, [FOOD_HOUSE_VAR], evidence, eliminationOrder)
    
    all_assignments=inferenceResult.getAllPossibleAssignmentDicts()
    max_p=0
    pos=None
    for assignment in all_assignments:
        if inferenceResult.getProbability(assignment)>max_p:
            max_p=inferenceResult.getProbability(assignment)
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        "*** YOUR CODE HERE ***"
        inferenceResult=inference.inferenceByVariableElimination(self.bayesNet, [FOOD_HOUSE_VAR, GHOST_HOUSE_VAR], evidence, eliminationOrder)
        assignments=inferenceResult.getAllPossibleAssignmentDicts()
        for assignment in assignments:
            p=inferenceResult.getProbability(assignment)
            if (assignment[FOOD_HOUSE_VAR]==TOP_LEFT_VAL and assignment[GHOST_HOUSE_VAR]==TOP_RIGHT_VAL):
                leftExpectedValue=WON_GAME_REWARD*p+GHOST_COLLISION_REWARD*(1-p)
            elif (assignment[FOOD_HOUSE_VAR]==TOP_RIGHT_VAL and assignment[GHOST_HOUSE_VAR]==TOP_LEFT_VAL):
                rightExpectedValue+=WON_GAME_REWARD*p+GHOST_COLLISION_REWARD*(1-p)
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        for prob, outcome in explorationProbsAndOutcomes:
