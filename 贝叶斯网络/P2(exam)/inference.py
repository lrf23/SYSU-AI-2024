...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        for var in eliminationOrder: # iterating over all the hidden variables, not over all the variables as in inferenceByEnumeration
            factors, new_factor = joinFactorsByVariable(factors, var) # (perform a join) join all of the factors that contain a variable
            if len(new_factor.unconditionedVariables()) > 1:
                temp_factor = eliminate(new_factor, var) # eliminate on a single hidden variable
                factors.append(temp_factor)
        final_factor = joinFactors(factors)
        return normalize(final_factor) # 输出的因子的概率总和应为 1
